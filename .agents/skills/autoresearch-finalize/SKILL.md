...
Key rules:
- **`last_commit` must be a full hash.** Expand from `autoresearch.jsonl` short hashes with `git rev-parse`.
- **No two groups may share a file.** `finalize.sh` validates this and fails if violated.
...
After the process finishes, report to the user:
- Branches created and what each contains
- Overall metric improvement (baseline → best)
- Cleanup commands or next-step commands for opening PRs

## Edge Cases

- **Only 1 kept experiment**: One branch is fine — don't force splits.
- **Overlapping files between groups**: merge the overlapping groups and retry.
