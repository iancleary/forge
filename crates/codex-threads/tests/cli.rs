...
fn run_codex_threads(args: &[&str]) -> std::process::Output {
    let bin = env!("CARGO_BIN_EXE_codex-threads");
    Command::new(bin)
        .args(args)
        .current_dir(repo_root())
        .output()
        .expect("run codex-threads")
}

#[test]
fn cli_parse_errors_honor_json_flag() {
    let output = run_codex_threads(&["--json", "events", "read"]);
...
fn cli_parse_errors_default_to_human_output() {
    let output = run_codex_threads(&["events", "read"]);
    assert!(!output.status.success());

    let stderr = String::from_utf8(output.stderr).expect("stderr utf8");
    assert!(stderr.contains("the following required arguments were not provided"));
    assert!(stderr.contains("Usage: codex-threads events read <SESSION_ID>"));
